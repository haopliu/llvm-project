...
SmallVector<std::pair<int64_t, int64_t>, 4>
  ...
  if (auto *CB = dyn_cast<CallBase>(W)) {
    ...
    if (Callee) {
      ...
      if (CalleeArg && CalleeArg->hasAttribute(Attribute::Initialized)) {
        SmallVector<std::pair<int64_t, int64_t>, 4> Result;
        for (const ConstantRange &Range : CalleeArg->getAttribute(Attribute::Initialized)
            .getValueAsRanges()) {
          Result.push_back(std::make_pair(Range.getLower().getSExtValue(), Range.getUpper().getSExtValue()));
        }
        return Result;
...
SmallVector<std::pair<int64_t, int64_t>, 4>
getWriteIntervals(const SmallVector<Instruction *, 8> Writes, Argument *Arg,
  ...
  SmallVector<std::pair<int64_t, int64_t>, 4> IntervalsInOrder;
...
SmallVector<std::pair<int64_t, int64_t>, 4>
getInitIntervals(const SmallVector<Instruction *, 8> &Writes,
                 const SmallVector<Instruction *, 8> &Reads, Argument *Arg,
  ...
  SmallVector<Instruction *, 8> Inits;
...
static SmallVector<std::pair<int64_t, int64_t>, 4>
determinePointerInitAttrs(Argument *A, SmallVector<Instruction *, 8> &Reads,
  ...
  SmallVector<Instruction *, 8> ReadsAndSpecialUses;
...
static bool addInitAttr(Argument *A,
                        ArrayRef<std::pair<int64_t, int64_t>> Inits) {
  assert(A && "Argument must not be null.");
  ...
  for (auto [Start, End] : Inits) {
    Ranges.push_back(ConstantRange(APInt(64, Start), APInt(64, End)));
  }

  A->addAttr(Attribute::get(A->getContext(), Attribute::Initialized, Ranges));
...
static void addArgumentAttrs(const SCCNodeSet &SCCNodes,
  ...
  for (scc_iterator<ArgumentGraph *> I = scc_begin(&AG); !I.isAtEnd(); ++I) {
    ...
    auto meetInitAttr =
        [](SmallVector<std::pair<int64_t, int64_t>, 4> &InitsA,
           SmallVector<std::pair<int64_t, int64_t>, 4> &InitsB) {
          SmallVector<std::pair<int64_t, int64_t>, 4> Intersection;
    ...
    SmallVector<std::pair<int64_t, int64_t>, 4> InitAttr;
    for (ArgumentGraphNode *N : ArgumentSCC) {
      Argument *A = N->Definition;
      Function *F = A->getParent();
      DominatorTree &DT = FAM.getResult<DominatorTreeAnalysis>(*F);
