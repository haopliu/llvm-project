...
// of arguments with "initializes" attribute.
class MemDefInfo {
public:
...
GetInitializesArgMemLoc(const Instruction *I, BatchAAResults &BatchAA) {
  ...
  for (const auto &[Idx, Start, End] : GetInitializesArguments(CB, BatchAA)) {
...
MemDefInfo::MemDefInfo(MemoryDef *MemDef, DSEState &State,
  ...
  if (consider_init_attr) {
    for (auto &MemLoc : GetInitializesArgMemLoc(DefInst, State.BatchAA)) {
